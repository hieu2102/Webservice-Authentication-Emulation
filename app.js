const http = require('http'), parser = require('body-parser');
var express = require('express'), app = express(), portApi = process.env.port || 80,fs = require('fs'), username, password;
...
app.post('/auth/jwt', (req, res)=>{
    ...
    if (inputUsername.trim() == username && inputPassword.trim() == password){
        var date = new Date();
        res.status(200).json({"tokem":"timeStamp_"+date.getTime()});
    }
});
...
app.post('/auth/test/jwt', (req, res)=>
{
    ...
    if (auth[0] == "Bearer"){
        ...
        if (Number(token[1]) + 5*60 <timeStamp){
            ...
            res.status(201).json({"result":"Failed authentication"});
        }
    }
});